 ...
 * CrossAndOut 编译时常量配置。
 * <p>
 * 与 {@link Config}（运行时 ModConfigSpec）不同，此类中的配置为编译时常量，
 * 适用于需要编译期确定的开关（如是否启用 SubLevel 缩放，这取决于引用的
 * Sable 版本是否支持 scale 传递到物理引擎原生层）。
 * <p>
...
public final class IACPConfig {
     ...
     * 是否启用 SubLevel 缩放。
     * <p>
     * 设为 {@code true} 需要改版 Sable（feature/sublevel-scale 分支），
     * 该版本在 JNI 层将 Pose3d.scale 传递到 Rapier 物理引擎原生层。
     * 使用原版 Sable 时必须设为 {@code false}，否则会导致 Java 侧变换
     * （座位高度、悬挂物理等）与物理碰撞体不同步。
     */

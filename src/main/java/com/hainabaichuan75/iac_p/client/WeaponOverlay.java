 ...
 *   <li>提供从当前摄像机位置出发的射线检测（最大 1000 格）：
 *       命中物包括实体、SubLevel 物理结构、传统 Minecraft 方块地形，均使用精确交点坐标</li>
...
public class WeaponOverlay {
            ...
            for (int x = localBounds.minX(); x <= localBounds.maxX(); x++) {
                for (int y = localBounds.minY(); y <= localBounds.maxY(); y++) {
                    for (int z = localBounds.minZ(); z <= localBounds.maxZ(); z++) {
                        ...
                        if (!(be instanceof TurretBaseBlockEntity tb)) continue;
                        if (!tb.isAssembled()) continue;
                        UUID rodId = tb.getLightningRodSubLevelId();
                        if (rodId == null) continue;
                        SubLevel sl = container.getSubLevel(rodId);
                        if (!(sl instanceof ClientSubLevel csl) || csl.isRemoved()) continue;
     ...
     * （自动忽略无碰撞箱的植物/农作物/藤曼等）+ 树叶穿透循环；
     * 实体使用 {@link AABB#clip(Vec3, Vec3)} 计算射线与碰撞箱的交点；
     * SubLevel 使用 AABB 算法交点。
     *
    ...
    private static Vec3 raycastGeneric(Minecraft mc, Vec3 origin, Vec3 dir, double maxDist,
        ...
        for (int pass = 0; pass < 32 && blockHit.getType() != HitResult.Type.MISS; pass++) {
            ...
            if (!isPenetrable(hitState)) break; // 非穿透方块 → 有效命中
        ...
        try {
            ...
            if (container != null) {
                for (SubLevel subLevel : container.getAllSubLevels()) {
                    if (!(subLevel instanceof ClientSubLevel csl)) continue;
                    UUID suuid = csl.getUniqueId();
                    // 排除自身载具和炮管 SubLevel
                    if (suuid.equals(excludeSubLevelUUID)) continue;
                    if (excludeSubLevelUUID2 != null && suuid.equals(excludeSubLevelUUID2)) continue;
     ...
     * 判断方块是否应被射线穿透（跳过）。
     * <p>
     * 配合 {@link ClipContext.Block#COLLIDER} 使用——碰撞箱为空的方块
     * （花/草/农作物/藤曼等）已被 COLLIDER 自然忽略，此方法仅处理
     * 有碰撞箱但应穿透的特殊方块（如树叶）。
     */
    ...
    private static Vec3 rayAABBIntersection(Vec3 origin, Vec3 dir,
        ...
        if (t > MAX_RAY_DISTANCE) return null;
